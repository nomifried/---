...
export const deleteOrder = async (req, res) => {
    let { id } = req.params; // מקבלים את ה-ID של ההזמנה מה-params של הבקשה

    try {
        // מנסים למצוא ולהמחות את ההזמנה לפי ה-ID
        let deletedOrder = await orderModel.findByIdAndDelete(id);

        // אם ההזמנה לא נמצאה, מחזירים שגיאה
        if (!deletedOrder) {
        ...
        // מחזירים את ההזמנה שנמחקה כהצלחה
        res.json({
...
export const updateOrderDispatched = async (req, res) => {
    ...
    try {
        // מנסים לעדכן את השדה isdispatched ל-true
        ...
        // מחזירים את ההזמנה שהעדכון הצליח
        res.json({
